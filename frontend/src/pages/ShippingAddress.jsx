...
export default function ShippingAddress() {
  ...
  const handleSubmit = async (e) => {
    ...
    // --- BƯỚC SỬA LỖI: KIỂM TRA ID CHẮC CHẮN 100% ---
    ...
    const payload = {
      orderCode: orderId,
      userId: Number(finalUserId), // Ép kiểu thành số cho chắc ăn
    ...
    try {
      const res = await fetch("http://localhost:5261/api/order", {
        method: "POST",
      ...
      alert("Hệ thống đang bận hoặc có lỗi, không thể đặt hàng lúc này!");
      setLoading(false);
    }
  };
          ...
          <form onSubmit={handleSubmit} style={s.form}>
            ...
            <div style={s.section}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                {[
                  {
                    id: "cod", icon: "💵", title: "COD — Thanh toán khi nhận hàng",
                    sub: "Kiểm tra hàng trước, thanh toán tiền mặt cho tài xế khi nhận",
                    badge: "Phổ biến nhất", badgeColor: "#eaf3de", badgeText: "#3b6d11",
                  },
                  {
                    id: "transfer", icon: "🏦", title: "Chuyển khoản ngân hàng",
                    sub: "Chuyển khoản trước khi giao hàng — nhận thông tin TK sau khi đặt", badge: null,
                  },
                ].map((method) => {
                    ...
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "3px" }}>
                          <span style={{ fontSize: "0.88rem", fontWeight: "700", color: active ? "#1a3c2e" : "#1a1a1a" }}>{method.title}</span>
                          {method.badge && (
                            <span style={{ fontSize: "0.65rem", fontWeight: "700", background: method.badgeColor, color: method.badgeText, padding: "2px 8px", borderRadius: "999px" }}>
                              {method.badge}
                            </span>
                          )}
                        </div>
                        <div style={{ fontSize: "0.78rem", color: "#aaa", lineHeight: "1.5" }}>{method.sub}</div>
                      </div>
                    </div>
                  );
                })}
              ...
              {form.paymentMethod === "transfer" && (
                <div style={{ background: "#e6f1fb", border: "1px solid #85b7eb", borderRadius: "10px", padding: "12px 16px", fontSize: "0.8rem", color: "#185fa5", lineHeight: "1.6" }}>
            ...
            <button type="button" style={s.backBtn} onClick={() => navigate("/checkout")}>
              ← Quay lại giỏ hàng
            </button>
          </form>
        </div>
            ...
            <div style={s.summaryItems}>
              {cartItems.map((item, idx) => (
                <div key={item.id} style={{ ...s.summaryRow, borderTop: idx === 0 ? "none" : "1px solid #f0eeea" }}>
                  ...
                  <div style={s.summaryPrice}>{(item.price * item.quantity).toLocaleString("vi-VN")} ₫</div>
                </div>
              ))}
            </div>
      ...
      {showLoginModal && (
        <div style={s.modalOverlay}>
          <div style={s.loginModal}>
            <div style={s.loginModalIcon}>🔒</div>
            <h2 style={s.loginModalTitle}>Yêu cầu đăng nhập</h2>
            <p style={s.loginModalText}>
              Bạn cần đăng nhập vào tài khoản để hệ thống lưu lại lịch sử đơn hàng và tiện theo dõi quá trình giao vật liệu nhé!
            </p>
            <div style={s.loginModalActions}>
              <button 
                style={s.loginBtn} 
...
const s = {
  page: { minHeight: "100vh", background: "#f5f4f0", padding: "40px 24px 80px", fontFamily: "'Be Vietnam Pro', 'Segoe UI', sans-serif" },
  progressWrap: { display: "flex", alignItems: "center", justifyContent: "center", gap: "0", marginBottom: "40px", maxWidth: "480px", margin: "0 auto 40px" },
  progressItem: { display: "flex", alignItems: "center", gap: "8px", position: "relative" },
  progressDot: { width: "28px", height: "28px", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.72rem", fontWeight: "800", flexShrink: 0, transition: "all 0.3s" },
  progressLabel: { fontSize: "0.82rem", whiteSpace: "nowrap" },
  progressLine: { width: "48px", height: "2px", borderRadius: "1px", margin: "0 8px" },
  layout: { maxWidth: "1100px", margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 340px", gap: "28px", alignItems: "start" },
  left: {}, right: {},
  form: { display: "flex", flexDirection: "column", gap: "20px" },
  section: { background: "#fff", borderRadius: "16px", border: "1px solid #ebebeb", padding: "28px", display: "flex", flexDirection: "column", gap: "18px" },
  sectionHead: { display: "flex", alignItems: "flex-start", gap: "14px", marginBottom: "4px" },
  sectionNum: { width: "28px", height: "28px", background: "#1a3c2e", color: "#a8d5b5", borderRadius: "8px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.8rem", fontWeight: "800", flexShrink: 0 },
  ...
  input: { flex: 1, border: "none", background: "transparent", padding: "12px 12px 12px 0", fontSize: "0.88rem", color: "#1a1a1a", width: "100%", fontFamily: "inherit" },
  errorMsg: { fontSize: "0.75rem", color: "#e24b4a", fontWeight: "600" },
  timeGrid: { display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "10px" },
  timeBtn: { padding: "14px 10px", border: "1.5px solid", borderRadius: "10px", cursor: "pointer", textAlign: "center", transition: "all 0.2s", fontFamily: "inherit" },
  submitBtn: { width: "100%", padding: "15px", background: "#1a3c2e", color: "white", border: "none", borderRadius: "12px", fontSize: "0.95rem", fontWeight: "700", transition: "all 0.2s", letterSpacing: "0.02em", fontFamily: "inherit" },
  backBtn: { width: "100%", padding: "12px", background: "transparent", color: "#666", border: "1.5px solid #d5d3cd", borderRadius: "12px", fontSize: "0.88rem", fontWeight: "600", cursor: "pointer", transition: "background 0.2s", fontFamily: "inherit" },
  ...
  summaryCard: { background: "#fff", borderRadius: "16px", border: "1px solid #ebebeb", overflow: "hidden" },
  summaryHead: { padding: "18px 22px", borderBottom: "1px solid #f0eeea", display: "flex", justifyContent: "space-between", alignItems: "center" },
  ...
  summaryRow: { display: "flex", alignItems: "center", gap: "12px", padding: "14px 0" },
  summaryImg: { width: "52px", height: "52px", borderRadius: "8px", background: "#f5f4f0", flexShrink: 0, overflow: "hidden" },
  ...
  grandRow: { display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: "10px", borderTop: "1px solid #f0eeea", marginTop: "4px" },
  ...
  successWrap: { maxWidth: "520px", margin: "60px auto", textAlign: "center", background: "#fff", borderRadius: "24px", border: "1px solid #ebebeb", padding: "56px 40px", animation: "popIn 0.4s ease" },
  successRing: { width: "72px", height: "72px", borderRadius: "50%", background: "#eaf3de", border: "3px solid #a8d5b5", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px" },
  successCheck: { fontSize: "1.8rem", color: "#3b6d11", fontWeight: "900" },
  successTag: { display: "inline-block", background: "#1a3c2e", color: "#a8d5b5", fontSize: "0.68rem", fontWeight: "700", letterSpacing: "0.12em", textTransform: "uppercase", padding: "4px 12px", borderRadius: "999px", marginBottom: "12px" },
  ...
  successCard: { background: "#faf9f7", borderRadius: "14px", border: "1px solid #ebebeb", padding: "20px", marginBottom: "28px", textAlign: "left", display: "flex", flexDirection: "column", gap: "10px" },
  ...
  successBtn: { padding: "13px 36px", background: "#1a3c2e", color: "white", border: "none", borderRadius: "10px", fontSize: "0.92rem", fontWeight: "700", cursor: "pointer", transition: "background 0.2s", fontFamily: "inherit" },

  // --- STYLE CHO POPUP YÊU CẦU ĐĂNG NHẬP ---
  modalOverlay: { position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 },
  loginModal: { background: "#fff", padding: "40px 32px", borderRadius: "20px", width: "100%", maxWidth: "400px", textAlign: "center", boxShadow: "0 20px 40px rgba(0,0,0,0.1)", animation: "popIn 0.3s ease" },
  loginModalIcon: { fontSize: "3rem", marginBottom: "16px" },
  loginModalTitle: { fontSize: "1.4rem", fontWeight: "800", color: "#1a1a1a", margin: "0 0 10px" },
  loginModalText: { fontSize: "0.9rem", color: "#666", lineHeight: "1.6", marginBottom: "28px" },
  loginModalActions: { display: "flex", flexDirection: "column", gap: "12px" },
  loginBtn: { padding: "14px", background: "#1a3c2e", color: "#fff", border: "none", borderRadius: "12px", fontSize: "0.95rem", fontWeight: "700", cursor: "pointer" },
  cancelBtn: { padding: "12px", background: "transparent", color: "#666", border: "1.5px solid #d5d3cd", borderRadius: "12px", fontSize: "0.9rem", fontWeight: "600", cursor: "pointer" },
};
