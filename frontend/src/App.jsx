...
// --- LOGIC BẢO VỆ ROUTE: Chỉ cho phép admin vào ---
...
function Navigation() {
  ...
  // Đồng bộ lại thông tin user mỗi khi chuyển trang (để cập nhật Avatar mới)
  useEffect(() => {
    const saved = localStorage.getItem("user");
  ...
  // Hàm xử lý hiển thị ảnh avatar chuẩn
    ...
    <nav style={{
      ...
      <div style={{ maxWidth: "1280px", margin: "0 auto", padding: "0 32px", height: "68px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
          ...
          {user?.role === "admin" && (
            <Link to="/admin" style={{
              color: "#ffc107", textDecoration: "none", fontSize: "0.88rem", fontWeight: "700",
              padding: "8px 16px", borderRadius: "8px", transition: "all 0.2s", marginLeft: "4px"
            }}
          ...
          {user ? (
            <div style={{ display: "flex", alignItems: "center", gap: "10px", marginLeft: "8px" }}>
              {/* NÚT AVATAR ĐỒNG BỘ - NHẤN ĐỂ VÀO PROFILE */}
              <Link to="/profile" style={{ textDecoration: "none" }}>
                <div style={{
                  ...
                  fontSize: "0.9rem", fontWeight: "800", color: "#a8d5b5", flexShrink: 0,
                  overflow: "hidden", transition: "all 0.3s", cursor: "pointer"
                }}
                  onMouseOver={(e) => { e.currentTarget.style.transform = "scale(1.1)"; e.currentTarget.style.borderColor = "#a8d5b5"; }}
                  onMouseOut={(e) => { e.currentTarget.style.transform = "scale(1)"; e.currentTarget.style.borderColor = "rgba(168,213,181,0.4)"; }}
                >
                  {user.avatar ? (
                    <img src={getAvatarSrc(user.avatar)} alt="Avatar" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                  ) : (
            ...
            <Link to="/login" style={{ color: "rgba(255,255,255,0.75)", textDecoration: "none", fontSize: "0.88rem", padding: "8px 16px", marginLeft: "8px" }}>
              Đăng nhập
            </Link>
          )}

          <Link to="/checkout" style={{
            display: "flex", alignItems: "center", gap: "8px", marginLeft: "12px", padding: "9px 18px",
          ...
          >
            ...
            {totalItems > 0 && (
              <span style={{
                background: "#d85a30", color: "white", fontSize: "0.68rem", fontWeight: "800",
...
function Footer() {
  return (
    <footer style={{ background: "#0c1912", borderTop: "1px solid rgba(168,213,181,0.1)", padding: "48px 32px 28px", fontFamily: "'Be Vietnam Pro', 'Segoe UI', sans-serif" }}>
      <div style={{ maxWidth: "1280px", margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "40px", marginBottom: "40px" }}>
        <div>
          ...
          <p style={{ color: "rgba(255,255,255,0.45)", fontSize: "0.85rem", lineHeight: "1.7", margin: 0 }}>Đối tác tin cậy cung cấp vật liệu xây dựng chất lượng cao cho mọi công trình.</p>
        </div>
        <div>
          ...
          {["Xi măng", "Sắt thép", "Gạch xây", "Cát & Đá"].map(item => (
            <div key={item} style={{ marginBottom: "8px" }}><Link to="/products" style={{ color: "rgba(255,255,255,0.5)", textDecoration: "none", fontSize: "0.88rem" }}>{item}</Link></div>
          ))}
        </div>
        <div>
          ...
          {[{ icon: "📞", text: "0909.123.456" }, { icon: "✉️", text: "kientao@vlxd.vn" }, { icon: "📍", text: "Đà Nẵng, Việt Nam" }].map(({ icon, text }) => (
            <div key={text} style={{ display: "flex", gap: "8px", marginBottom: "10px", alignItems: "flex-start" }}>
              <span style={{ fontSize: "0.9rem" }}>{icon}</span><span style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.88rem" }}>{text}</span>
            </div>
          ))}
        </div>
      </div>
